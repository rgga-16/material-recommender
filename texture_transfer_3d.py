...
if __name__=="__main__":
    ...
    prompt = "white marble texture map, 4k"
    images = []
    sample_num = 1
    ...
    for i in range(sample_num):
        with autocast("cuda"):
            ...
            '''
            output_dict = pipe(prompt, width=512,height=512,guidance_scale=7.5,prompt_strength=1.0,num_inference_steps=50)
            image = output_dict["sample"][0]
            text_impath = f"texture_{i}.png"
        ...
        # command_str = f'blender --python rendering/blender.py -- --mesh_folder {mesh_folder} --texture {text_path} --renderfolder {rendering_folder}'
        print(command_str)
